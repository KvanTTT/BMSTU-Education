...
{
    public sealed class KMeansAlgorithm
    {
        /// <summary>
        ...
        public KMeansAlgorithm(List<ClusterPoint> points, List<ClusterCentroid> clusters, float fuzzy)
        {
            if (points == null)
            {
        ...
        private void RecalculateClusterIndexes() 
        {
            for (int i = 0; i < this.Points.Count; i++)
            {
                ...
                for (int j = 0; j < this.Clusters.Count; j++)
                {
                    ...
                    {
                        max = U[i, j];
                        p.ClusterIndex = (float)((max == 0.5) ? 0.5 : j);
                    }
                }
            }
        }
