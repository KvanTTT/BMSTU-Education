...
/*
    This is a free version of the file ntifs.h, release 30.
    The purpose of this include file is to build file system and
    file system filter drivers for Windows NT® and Windows® 2000.
    ...
    Thanks to:
      ...
      Emanuele Aliberti, Anton Altaparmakov, Dan Partelly, Mamaich, Yossi Yaffe,
      Gunnar André Dalsnes, Vadim V Vorobev and Darja Isaksson.
...
typedef struct _FILE_LOCK_ANCHOR {
    ULONG           Unknown1;
    ULONG           Unknown2;
    BOOLEAN         TrueIfTocIsInitialized;
    PVOID           FileLockToc; 
    FILE_LOCK_ENTRY GetNextFileLock_Lock;
    PVOID           GetNextFileLock_RealLock;
...
FsRtlFastUnlockAll (
    ...
    IN PFILE_OBJECT         FileObject,
    IN PEPROCESS            Process,
    PVOID                   AllwaysZero     //unknown; set to zero
);
...
FsRtlFastUnlockSingle (
    ...
    IN PEPROCESS            Process,
    IN ULONG                Key,
    PVOID                   AllwaysZero,    //unknown; allways zero
    PVOID                   UnusedInKernel  //unused in kernel; set to zero
);                      
//ret:  STATUS_RANGE_NOT_LOCKED
...
/*
  ...
  Internals:
    FsRtlGetNextFileLock uses FileLockAnchor->GetNextFileLock_Lock and
    FileLockAnchor->GetNextFileLock_RealLock as storage.
    GetNextFileLock_RealLock is a pointer to the 'raw' lock inkl. double linked
    list, and FsRtlGetNextFileLock needs this to get next lock on subsequent
    calls with firstRun = FALSE.
*/
NTKERNELAPI
PFILE_LOCK_ENTRY
FsRtlGetNextFileLock (
    IN PFILE_LOCK_ANCHOR    FileLockAnchor,
    IN BOOLEAN              TrueOnFirstRun_FalseOnSubsequentRuns
);
...
FsRtlInitializeFileLock (
    IN PFILE_LOCK_ANCHOR    FileLockAnchor,
    IN PVOID                Unknown1,   // Placed in Unknown1 in FileLockAnchor
    IN PVOID                Unknown2    // Placed in Unknown2 in FileLockAnchor
);
...
FsRtlPrivateLock (
    ...
    OUT PIO_STATUS_BLOCK    IoStatus, 
    IN PIRP                 Irp OPTIONAL,   //zero if fast io?
    PVOID                   AllwaysZero,    //unknown; set to zero
    PVOID                   UnusedInKernel  //unused in kernel; set to zero
);
...
ObQueryNameString (
    IN PVOID                        Object,
    IN OUT POBJECT_NAME_INFORMATION Name,
